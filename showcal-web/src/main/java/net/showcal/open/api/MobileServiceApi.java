...
import com.showcal.mobile.request.VerificationCodeCreateRequest;
...
public class MobileServiceApi {
    ...
    @ServiceMethod(title = "创建验证码", method = "mobile.create.code", httpAction = HttpAction.POST, version = "1.0")
    public Object createCode(AbstractApiRequest apirequest) {

        Passport passport = requestConverter.requestToPassport(apirequest);
        VerificationCodeCreateRequest verificationCodeCreateRequest = requestConverter.converter(VerificationCodeCreateRequest.class, apirequest);
        verificationCodeCreateRequest.setVerificationType(VerificationTypeEnum.MOBILE);
        HttpServletRequest request = (HttpServletRequest) apirequest.getApiRequestContext().getRawRequestObject();
        ErrorResponse response = requestConverter.doValidate(request, verificationCodeCreateRequest);
        if (response != null) {
            return response;
        } else {
            SessionUtil.remove(String.valueOf(passport.getId()));
            return mobileService.createCode(verificationCodeCreateRequest,passport);
        }
    }
